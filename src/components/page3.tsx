  ...
  return (
    <Stack id={id} className="page3">
      ...
      <div className="cardgrid">
        {vrCards.map((card) => (
          <div className="clip-wrapper" key={card.id}>
            <div className={openCardId === card.id ? "cardblock expanded " : "cardblock collapsed"}>
              <img src={card.image} className={openCardId === card.id ? "media-colored" : "media-grayscale"} />
              {/* Collapsible Content */}
              {openCardId === card.id && (
                <div className="description">
                  {card.description}
                  <Link to={card.link} className="explore-button">Explore more..</Link>
                </div>
        ...
        <div className="clip-wrapper">
          <div className={openCardId === 4 ? "cardblock expanded" : "cardblock collapsed"}>
            <video src="./assets/video1.mp4"
              autoPlay
              loop
              muted
              playsInline
              className={openCardId === 4 ? "media-colored" : "media-grayscale"}
            />
            {openCardId === 4 && (
              <div className="description">
          ...
          <div className="label_Wrap">
            <div className="label_Wrap_inner">
              <div className="label">
                ...
                <button className="cta" aria-label="Open" onClick={() => toggleCard(4)}>
                  {openCardId === 4 ? (
                    <svg className="icon-open" viewBox="0 0 24 24" aria-hidden="true">
                      <path
                        d="M7 17l8-8M9 7h8v8"
